...
MODULE ID           : task1_collector-12-RC3.py
VERSION             : v5.8.2-AbsoluteObedience (絕對服從總裁數據暨CMMI5完全體)
...
- 完全拔除終端控制台列印（Console Print）：
  ...
  RC3 - 1. 更新 API Key 
        2. 風控降溫防線：只要出錯，不管是 503 還是 429，原地強制裝死 10 秒
...
# =====================================================================
GEMINI_API_KEYS = [
    os.environ.get("GEMINI_API_KEY_1", "AQ.Ab8RN6I5KXZ6X-l-V5v-Tf77Bb0lZCZQDKk0VdbXBqQJigKy1Q"),  #mao.yuang
    os.environ.get("GEMINI_API_KEY_2", "AQ.Ab8RN6IKLnoTXKUpbU0GuJZYGp7mhaXF1ZW6VWE3AnmC6vGeIA"),  #tok.chen
    os.environ.get("GEMINI_API_KEY_3", "AQ.Ab8RN6JAnRhuiKR2O6qNfwdTi7nY82SF_GhZe0wuuEVs3ez4lw"),  #changyuyuan3@gmail.com
    os.environ.get("GEMINI_API_KEY_4", "AQ.Ab8RN6Lhmf2ANoxH5rdUrjz8Yh1zHi9jw-8WTStKf4CmYHlTsA"),  #yuyuanmao672@gmail.com
]
...
def call_gemini_with_double_shield(prompt, tools_config=None, required_keywords=None, forbidden_keywords=None):
    ...
    while key_index < len(GEMINI_API_KEYS):
        ...
        for attempt in range(2):
            ...
            # 🛡️ CMMI Level 5 風控降溫防線：只要出錯，不管是 503 還是 429，原地強制裝死 10 秒！
                logger.warning(f"⚠️ 請求遭遇波動 (錯誤: {e})，啟動 10 秒鐘戰術冷卻緩衝...")
                time.sleep(10)
